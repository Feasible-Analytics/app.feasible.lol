...
test("a pageview carries exactly the documented keys", async ({ page }) => {
  ...
  // The viewport width rides on every event. It is what the screen-size
  // report is built from, and a browser reporting an impossible one is one of
  // the ways an automated visitor gives itself away.
  ...
  // signal is ever written to fire on an ordinary visitor.
...
// one of those claims.
test("a browser with no window at all is reported as automated", async ({
  page,
}) => {
...
// A browser that hides its platform is a privacy build, not a robot. Several
// ship that way by default, and the people who choose them are exactly the
// people most likely to be reading a site that chose us.
test("a browser that hides its platform is left alone", async ({ page }) => {
  await page.addInitScript(() => {
...
test("events are posted as text/plain, which needs no CORS preflight", async ({
  page,
}) => {
  const state = await collect(page);
...
test("revenue is sent under the dollar key", async ({ page }) => {
  ...
  expect(named(state, "Purchase")[0].$).toEqual({
    amount: 42.5,
    currency: "USD",
  });
});
...
test("events queued before the bundle loads are replayed under every API name", async ({
  page,
}) => {
  const state = await collect(page);
...
test("a failed event is kept and retried on the next pageview", async ({
  page,
}) => {
  const state = await collect(page);
  ...
  expect(
    await page.evaluate(() => localStorage.getItem("feasible_outbox")),
  ...
  await waitFor(
    state,
    (events) =>
      events.filter((e) => e.n === "pageview" && e.u.includes("basic.html"))
        .length >= 2,
  ...
  const attempts = state.events.filter(
    (event) => event.n === "pageview" && event.u.includes("basic.html"),
  );
...
test("a lost 202 retries with the same client idempotency key", async ({
  page,
}) => {
  const state = await collect(page);
  state.disconnect = 1;
  ...
  await expect
    .poll(() =>
      page.evaluate(
        () =>
          JSON.parse(localStorage.getItem("feasible_outbox") || "[]").length,
      ),
    )
    .toBe(1);
  ...
  await waitFor(
    ...
    (events) =>
      events.filter(
        (event) =>
          event.n === "pageview" && event.u && event.u.includes("basic.html"),
      ).length >= 2,
    "lost-202 replay",
  );
...
test("a cancelled request was queued before fetch and retries with the same key", async ({
  page,
}) => {
  const state = await collect(page);
  state.cancel = 1;
...
test("storage failure is explicit and keeps the tracker running", async ({
  page,
}) => {
  await page.addInitScript(() => {
  ...
  await settledCount(state, "pageview", 1);
  await expect
    .poll(() => warnings.some((message) => message.includes("memory-only")))
    .toBeTruthy();
