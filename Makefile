...
ui-css: web-deps
	@if [ -x web/node_modules/.bin/tailwindcss ]; then \
		NODE_PATH="$(CURDIR)/web/node_modules" web/node_modules/.bin/tailwindcss \
			-i internal/assets/tailwind.css -o internal/assets/ui/app.css --minify; \
	elif command -v tailwindcss >/dev/null 2>&1; then \
		tailwindcss -i internal/assets/tailwind.css -o internal/assets/ui/app.css --minify; \
	else \
		echo "tailwindcss is not installed — keeping the committed internal/assets/ui/app.css"; \
	fi
