...
// Command gen-lists writes the classification data that ships inside the
// binary: the address ranges each cloud provider publishes about itself, and
// the current version of each browser that updates itself. It is a build-time
// tool, never embedded in the product, and its output is committed so that
// `go build` works from a clean checkout with no network.
//
...
func run(out string, allowShrink bool) error {
	...
	// The version floor is written from the same run, and a source that fails
	// here joins the same roll call. Left out of it, the largest source could go
	// missing without the coverage check being able to see it.
	if err := browsers(ctx, client, filepath.Join(out, "browsers.txt")); err != nil {
		fmt.Fprintln(os.Stderr, "  !", err)
		failed = append(failed, "browser versions")
	}
...
func browsers(ctx context.Context, client *http.Client, out string) error {
	...
	body.WriteString("#\n")
	body.WriteString("# Safari is absent because its version follows the operating system, so a\n")
	body.WriteString("# supported phone sits several majors back and is still a person. Firefox is\n")
	body.WriteString("# absent because its extended-support channel, every Linux distribution's\n")
	body.WriteString("# package and Tor Browser are far behind on purpose and permanently.\n")
	body.WriteString("\n")

	known := lists.CurrentBrowsers()
	...
	for _, source := range lists.BrowserSources() {
		...
		// Browsers do not go backwards. A lower number than the one already
		// committed is a changed document being read wrong, and writing it
		// would lower the floor for everybody without a word.
		if was, ok := known[source.Name]; ok && major < was {
			return fmt.Errorf("%s came back as %d but the committed list says %d", source.Name, major, was)
		}
