...
type BehaviorTab = "goals" | "properties" | "funnels" | "explore";
...
export function GoalsCard({ domain, range, filters, exact: exactAnswer, onFilter }: Props) {
	const [ref, near] = useNearViewport<HTMLElement>();
	const [tab, setTab] = useState<BehaviorTab>("goals");
	const [expanded, setExpanded] = useState(false);
	const request = useMemo(() => ({ dateRange: range, filters, exact: exactAnswer }), [range, filters, exactAnswer]);
	const settingsURL = conversionSettingsURL(bootstrap().navigation?.site_settings_url);
	...
	return (
		...
		>
			...
			<div className="min-h-[350px] flex-1">
				...
				{tab === "properties" && (
					<PropertiesPanel domain={domain} request={request} enabled={near} onFilter={onFilter} settingsURL={settingsURL} />
				)}
				{tab === "funnels" && (
					<FunnelsPanel domain={domain} request={request} enabled={near} settingsURL={settingsURL} />
				)}
				{tab === "explore" && <ExplorePanel domain={domain} request={request} enabled={near} />}
			</div>
		</section>
	);
}
...
function PropertiesPanel({ domain, request, enabled, onFilter, settingsURL }: { domain: string; request: { dateRange: DateRange; filters: Filter[]; exact: boolean }; enabled: boolean; onFilter: Props["onFilter"]; settingsURL?: string }) {
	const list = useRemote<Property[]>(domain, enabled, (signal) => properties(domain, signal));
	const [selected, setSelected] = useState("");

	useEffect(() => {
		const available = list.data ?? [];
		if (available.length === 0) setSelected("");
		else if (!available.some((property) => property.name === selected)) setSelected(available[0]?.name ?? "");
	}, [domain, list.data, selected]);

	const report = useRemote<PropertyReport>(JSON.stringify({ domain, selected, request }), enabled && Boolean(selected), (signal) => propertyReport(domain, selected, request, signal));
...
function FunnelsPanel({ domain, request, enabled, settingsURL }: { domain: string; request: { dateRange: DateRange; filters: Filter[]; exact: boolean }; enabled: boolean; settingsURL?: string }) {
	const list = useRemote<Funnel[]>(domain, enabled, (signal) => funnels(domain, signal));
	const [selected, setSelected] = useState(0);

	useEffect(() => {
		const available = list.data ?? [];
		if (available.length === 0) setSelected(0);
		else if (!available.some((funnel) => funnel.id === selected)) setSelected(available[0]?.id ?? 0);
	}, [domain, list.data, selected]);

	const report = useRemote<FunnelReport>(JSON.stringify({ domain, selected, request }), enabled && selected > 0, (signal) => funnelReport(domain, selected, request, signal));
	...
	return (
		<PanelFrame footer={settingsURL ? <a href={settingsURL} className="text-xs font-medium text-muted transition-colors hover:text-accent">{t("dashboard.behavior.funnels.manage")} →</a> : undefined}>
			<SelectorBar label={t("dashboard.behavior.funnels.selector_label")} value={String(selected)} onChange={(value) => setSelected(Number(value))}>
...
function ExplorePanel({ domain, request, enabled }: { domain: string; request: { dateRange: DateRange; filters: Filter[]; exact: boolean }; enabled: boolean }) {
	...
	const [anchor, setAnchor] = useState<JourneyAnchor | null>(null);
	const [direction, setDirection] = useState<JourneyDirection>("forward");
	const [trail, setTrail] = useState<JourneyAnchor[]>([]);
	...
	useEffect(() => {
		if (!anchor && allOptions.length > 0) setAnchor(allOptions[0] ?? null);
	}, [allOptions, anchor]);

	const report = useRemote<JourneyReport>(JSON.stringify({ domain, anchor, direction, trail, request }), enabled && Boolean(anchor), (signal) => journeyReport(domain, anchor as JourneyAnchor, direction, trail, request, signal));
	...
	const chooseAnchor = (key: string) => {
		setAnchor(allOptions.find((item) => anchorKey(item) === key) ?? allOptions[0] ?? null);
		setTrail([]);
	};

	/** continueTo appends the current node before following the selected result. */
	const continueTo = (next: JourneyAnchor) => {
		if (anchor) setTrail((current) => extendJourneyTrail(current, anchor));
		setAnchor(next);
	};
	...
	return (
		<PanelFrame>
			<div className="flex flex-col gap-2 border-b border-line px-4 py-3 sm:flex-row sm:items-end sm:px-5">
				<label className="min-w-0 flex-1 text-[11px] font-medium tracking-wide text-muted uppercase">{t("dashboard.behavior.explore.start")}<select value={anchor ? anchorKey(anchor) : ""} onChange={(event) => chooseAnchor(event.target.value)} className="mt-1 block h-control w-full rounded-md border border-line bg-card px-2.5 text-sm text-body"><AnchorGroup label={t("dashboard.behavior.explore.pages")} anchors={options.data.pages} /><AnchorGroup label={t("dashboard.behavior.explore.events")} anchors={options.data.events} /><AnchorGroup label={t("dashboard.behavior.explore.goals")} anchors={allOptions.filter((item) => item.type === "goal")} /></select></label>
				<div className="flex h-control rounded-md border border-line bg-page p-0.5" aria-label={t("dashboard.behavior.explore.direction")}>{(["backward", "forward"] as JourneyDirection[]).map((candidate) => <button key={candidate} type="button" aria-pressed={direction === candidate} onClick={() => { setDirection(candidate); setTrail([]); }} className={`rounded-sm px-3 text-xs transition-colors ${direction === candidate ? "bg-card font-medium text-body shadow-sm" : "text-muted hover:text-body"}`}>{journeyDirectionLabel(candidate)}</button>)}</div>
			</div>
...
function NumberCell({ value }: { value: number }) {
	return <span className="tnum pointer-events-none relative text-right text-sm text-body" title={exact(value)}>{compact(value)}</span>;
}
...
/** conversionSettingsURL lands configuration links on the relevant subsection
 * and avoids a double slash when the server's base URL already ends in one. */
export function conversionSettingsURL(base?: string): string | undefined {
	return base ? `${base.replace(/\/+$/, "")}/conversions` : undefined;
}
