//
// litestream.go
// The replication configuration for control.db and every account database on a box.
//
// Created: 2026-08-31
...
// Plan lists every database that must be replicated right now: the control
// database, then one per account directory that exists on disk.
//
// The account list comes from the disk rather than from control.db on purpose.
// This is the same choice the migrate and backup commands make, and for the
// same reason: a box whose control database is unreadable is exactly when
...
func Plan(opts Options) ([]Database, error) {
	...
	plan := []Database{{
		...
		Name:       ControlName,
		ReplicaURL: opts.ReplicaURL + "/" + ControlName,
	}}
...
func Render(plan []Database, opts Options) []byte {
	...
	b.WriteString("#\n")
	b.WriteString("# One entry per database: control.db, then one per account. A new account\n")
...
type Result struct {
	...
	// Databases is how many entries the file now has, control included.
	Databases int
