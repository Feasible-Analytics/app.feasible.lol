...
func propAggregateMetric(name string) (metric, bool) {
	...
	definition := metric{
		...
		// A total scales with the sample; an average, a minimum, a maximum and
		// a percentile are all statements about the values themselves and are
		// unchanged by reading fewer of them.
...
func (x *executor) propValueQuery(_ context.Context, parsed propAggregate, r Resolved, keys map[int][]any) (expr, []compiledDim, error) {
	...
	if parsed.Dim.sessionScoped(x.plan.Scopes) {
		visit := sessionPropExpr(parsed.Dim, "e.session_id")

		// The visit's own value, collapsed to one row per visit. MIN over a
		// value that is constant inside the group is that value; it is here
		// only because SQL needs an aggregate over a column it grouped by.
		value = expr{SQL: "MIN(" + NumberFunction + "(" + visit.SQL + "))", Args: visit.Args}
		groupExtra = []string{"e.session_id"}
		ungrouped = false
	} else {
	...
	st := statement{
		...
		groupExtra: groupExtra, ungrouped: ungrouped,
	}
