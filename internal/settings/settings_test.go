...
// layoutHasNoProse names the one template that legitimately calls nothing.
//
// The layout is chrome the screens render into: the settings shell's own
// wording comes from the account surface's template, and this file carries
// only structure. It is named here rather than skipped by a rule, so adding a
// second exception is a decision somebody has to write down.
const layoutHasNoProse = "layout.html"

// TestEverySettingsScreenUsesTheCatalogue is the guard against a screen
// shipping in one language inside a translated product.
//
// A screen with no catalogue calls is also invisible to the i18n coverage
// tests, which check that every id in use has a string and every string is
// used. A file they cannot see is a file where those guarantees do not apply,
// so "no calls at all" is the condition worth failing on.
func TestEverySettingsScreenUsesTheCatalogue(t *testing.T) {
	...
	for _, entry := range entries {
		...
		if entry.Name() == layoutHasNoProse {
			continue
		}
		...
		if !strings.Contains(string(body), "t $.Lang \"") && !strings.Contains(string(body), "n $.Lang \"") {
			t.Errorf("%s renders no catalogue string, so every word on it is English "+
				"and the coverage tests cannot see the file at all", entry.Name())
		}
	}

	if screens == 0 {
		t.Fatal("no settings templates were read, so this test proves nothing")
	}
}
